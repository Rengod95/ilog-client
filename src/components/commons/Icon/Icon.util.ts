import { ViewPortObject, ViewBoxObject } from '.';
import { ICON_SET, IconName, ViewPortSize, ViewBoxSize, IconData } from '.';

export const DEFAULT_VIEW_PORT_SIZE: ViewPortObject = {
  width: 100,
  height: 100,
};
export const DEFAULT_VIEW_BOX_SIZE: ViewBoxObject = {
  minX: 0,
  minY: 0,
  width: 24,
  height: 24,
...
// ViewBox Size를 결정해주는 함수, icon 자체의 ViewBoxSisze가 아닌 임의의 Size를 입력할 수 있도록 타입체킹
// 임의의 size 값을 전달받은경우 최우선으로 적용하도록 작성 외에는 iconName에 따른 사이즈 값 반환
export const setViewBoxSize = (
  size: ViewBoxSize,
  iconName: IconName
): ViewBoxObject => {
